...
chain = llmresponse.load_chain(st.session_state.memory)
...
if prompt := st.chat_input("Record Audio"):
    # Initialize audio recording
    with st.chat_message("user"):
        st.markdown(prompt)

    audio = audiohandling.process_audio()
    
    # Store response in session_state
    st.session_state.chat_history.append({"role": "assistant", "content": audio.text})
